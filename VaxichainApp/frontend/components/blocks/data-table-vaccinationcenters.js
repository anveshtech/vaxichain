...
const DisableModal = ({ isOpen, onClose, onSubmit, vaccinationCenter, loading }) => {
  const [remarks, setRemarks] = useState("")

  const handleDisableSubmit = () => {
    onSubmit(vaccinationCenter, remarks)
    setRemarks("")
    onClose()
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="w-[30rem] rounded-md bg-white p-6 shadow-lg">
        ...
        <h2 className="text-lg font-bold">Disable vaccinationCenter</h2>
        <p className="mt-2">Please provide a reason for disabling the vaccinationCenter:</p>
        <textarea
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="w-[20rem] rounded-md bg-white p-6 shadow-lg">
        <h2 className="text-lg font-bold">Approve vaccinationCenter</h2>
        <p className="mt-2">Do you want to approve this vaccinationCenter?</p>
...
export default function DataTable() {
  ...
  const [vaccinationCenterToDisable, setVaccinationCenterToDisable] = useState(null) // Track which vaccinationCenter to disable
  const [vaccinationCenterToApprove, setVaccinationCenterToApprove] = useState(null)
  ...
  //   setLoading(true)
  //   const updatedVaccinationCenter = { ...vaccinationCenterToApprove, vaccinationStatus: "enabled" }

  //   try {
  //     await updateVaccinationCenterStatus(vaccinationCenterToApprove.id, updatedVaccinationCenter)
  //     await fetchVaccinationCenters()
  //     setIsApprovalModalOpen(false)
  //   } catch (error) {
  //     console.error("Failed to approve vaccinationCenter:", error)
  //   } finally {
  //     setLoading(false)
  //   }
  // }

  const handleDisable = (vaccinationCenter) => {
    setVaccinationCenterToDisable(vaccinationCenter)
    setIsModalOpen(true) // Open modal
  }
  const handleSubmitDisable = async (vaccinationCenter, remarks) => {
    setLoading(true)
    const updatedVaccinationCenter = { ...vaccinationCenter, vaccinationStatus: "disabled", remarks }

    try {
      await updateVaccinationCenterStatus(vaccinationCenter.id, updatedVaccinationCenter)
      await fetchVaccinationCenters()
    } catch (error) {
      console.error("Failed to disable vaccinationCenter:", error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const updateVaccinationCenterStatus = async (id, updatedVaccinationCenter) => {
    try {
      ...
      console.error("Error updating vaccinationCenter vaccinationStatus:", error.response?.data || error.message)
      throw new Error("Failed to update vaccinationCenter vaccinationStatus")
    }
  }
  const handleDelete = async (vaccinationCenterId) => {
    if (window.confirm("Are you sure you want to delete this vaccinationCenter?")) {
      try {
        await deleteDataverifier(dataverifierId)
        await fetchVaccinationCenters()
      } catch (error) {
        console.error("Failed to delete vaccinationCenter:", error)
      }
    }
  }
  ...
  return (
    <div className="space-y-4">
            ...
            {dataLoading ? (
              ...
              filteredData?.map((datum, idx) => (
                ...
                >
                  ...
                  <Table.Column className="px-2">{datum?.vaccinationCenterName} </Table.Column>

                  <Table.Column className="overflow-hidden p-2">
                    <span className="line-clamp-1">{datum.observerDetail.observerName}</span>
                  </Table.Column>
                  <Table.Column className="overflow-hidden p-2">
                    <span className="line-clamp-1">{datum.observerDetail.observerPhone}</span>
                  </Table.Column>
                  ...
                  <Table.Column className="p-2">
                    <span
                      className={twMerge(
                        ...
                        datum.vaccinationCenterStatus === "Stopped"
                          ? "bg-red-500"
                          : datum.vaccinationCenterStatus === "Ongoing" || datum.vaccinationCenterStatus === "Ongoing"
                            ? "bg-green-600"
                            : "bg-gray-500",
                      )}
                    >
                      {datum.vaccinationCenterStatus}
                    </span>
                  </Table.Column>
                  <Table.Column className="p-2">
                        ...
                        <>
                          {userRole === "dataCollector" && (
                            <ContextMenu.Item
                              ...
                              onClick={() => router.push(`/vaccinationcenters/${datum.id}/edit`)}
                            >
                              Edit
                            </ContextMenu.Item>
                          )}
                          {userRole === "dataCollectorUser" && (
                            <ContextMenu.Item
                              className="rounded-md bg-purple-900"
                              onClick={() => router.push(`/vaccinationcenters/${datum.id}/children`)}
                            >
                              Manage Children
                            </ContextMenu.Item>
                          )}
                          ...
                          {/* <ContextMenu.Item
                              className="rounded-md bg-purple-900"
                              onClick={() => {
                                setVaccinationCenterToApprove(datum)
      ...
      <div className="text-right">
        <Pagination
          totalNumberOfData={data?.pagination?.totalCenters || 0}
      ...
      <DisableModal
        ...
        vaccinationCenter={vaccinationCenterToDisable}
        loading={loading}
      />
      {/* <ApprovalModal
