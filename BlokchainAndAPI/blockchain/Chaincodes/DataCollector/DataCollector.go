...
func (dc *DataCollector) Init(stub shim.ChaincodeStubInterface) peer.Response {
	fmt.Println("Initializing the chaincode")
	return shim.Success(nil)
}

func (dc *DataCollector) GetCentersWithHashes(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	// Implementation for GetCentersWithHashes
	return shim.Error("Not implemented yet")
}

func (dc *DataCollector) QueryVaccinationCenter(stub shim.ChaincodeStubInterface, rcID string) (VaccinationCenter, error) {
...
func (dc *DataCollector) invokeAnotherChaincode(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	if len(args) < 2 {
...
func (dc *DataCollector) getListOfAssignedVacCenterCollector(stub shim.ChaincodeStubInterface, userID string) peer.Response {
	...
	for _, center := range vaccinationCenters {
		// Check if the center has the assigned Data Collector user
		if center.Dc_orguser.Id == userID && center.Dc_orguser.UserType == "Data Collector" {
	...
	if len(assignedCenters) == 0 {
		return shim.Error(fmt.Sprintf("No vaccination centers assigned to the Data Collector with userID: %s", userID))
	}
...
func (dc *DataCollector) Invoke(stub shim.ChaincodeStubInterface) peer.Response {
	...
	case "GetCentersWithHashes":
		...
		return dc.GetCentersWithHashes(stub, args)
	...
	case "QueryVaccinationCenter":
		...
		vaccinationCenter, err := dc.QueryVaccinationCenter(stub, args[0])
		if err != nil {
	...
	case "invokeAnotherChaincode":
		...
		return dc.invokeAnotherChaincode(stub, args)
...
func main() {
	err := shim.Start(new(DataCollector))
	if err != nil {
